...
export default function BasicPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      // Load course settings - using basic API endpoint
      const settingsRes = await fetch('/api/admin/basic/course-settings', { credentials: 'include' });
      if (settingsRes.ok) {
      ...
      const [recipesRes, mealsRes, weekMetasRes, knowledgeRes] = await Promise.all([
        fetch('/api/recipes/search?tags=functional-basics', { credentials: 'include' }),
        fetch('/api/admin/meal-plans?course=basic', { credentials: 'include' }),
        fetch('/api/admin/course-weeks?course=basic', { credentials: 'include' }),
        fetch('/api/admin/knowledge/course-documents?course=basic', { credentials: 'include' })
      ]);
  ...
  const loadShoppingList = async (week: number) => {
    try {
      const res = await fetch(`/api/admin/shopping-lists?course=basic&week=${week}`, {
        credentials: 'include'
      });
      if (res.ok) {
  ...
  const renderShoppingListView = () => {
    ...
    const saveShoppingList = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/admin/shopping-lists/basics/${selectedWeek}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({
        ...
        <form onSubmit={(e) => {
          ...
          <div>
            ...
            <input
              ...
              placeholder="T.ex. Grunderna i funktionell mat"
          ...
          <div>
            ...
            <input
              ...
              placeholder="T.ex. Introduktion till råvaror"
              className="admin-input"
            />
          </div>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Functional Basics</h1>
          ...
          {activeView === 'overview' && (
            <div className="admin-card max-w-2xl">
              ...
              <form onSubmit={async (e) => {
                ...
                try {
                  const formData = new FormData(e.currentTarget);
                  const response = await fetch('/api/admin/basic/course-settings', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({
