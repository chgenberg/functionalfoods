...
export default function EnergyPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const settingsRes = await fetch('/api/admin/energy/course-settings', { credentials: 'include' });
      if (settingsRes.ok) {
      ...
      const [recipesRes, mealsRes, weekMetasRes, knowledgeRes] = await Promise.all([
        fetch('/api/recipes/search?tags=functional-energy', { credentials: 'include' }),
        fetch('/api/admin/meal-plans?course=energy', { credentials: 'include' }),
        fetch('/api/admin/course-weeks?course=energy', { credentials: 'include' }),
        fetch('/api/admin/knowledge/course-documents?course=energy', { credentials: 'include' })
      ]);
  ...
  const loadShoppingList = async (week: number) => {
    try {
      const res = await fetch(`/api/admin/shopping-lists?course=energy&week=${week}`, {
        credentials: 'include'
      });
      if (res.ok) {
        ...
        <form onSubmit={(e) => {
          ...
          <div>
            ...
            <input
              ...
              placeholder="T.ex. Energi och prestanda"
          ...
          <div>
            ...
            <input
              ...
              placeholder="T.ex. Introduktion till energioptimering"
              className="admin-input"
            />
          </div>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Functional Energy</h1>
          ...
          {activeView === 'overview' && (
            <div className="admin-card max-w-2xl">
              ...
              <form onSubmit={async (e) => {
                ...
                try {
                  const formData = new FormData(e.currentTarget);
                  const response = await fetch('/api/admin/energy/course-settings', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({
