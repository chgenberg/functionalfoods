...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const recipe of recipes) {
      if (!recipe.tags.includes('functional-basics')) {
        await prisma.recipe.update({
          where: { id: recipe.id },
          data: { tags: { push: 'functional-basics' } }
        });
        taggedCount++;
      }
    }
    ...
    console.error('Admin basic tag recipes error:', error);
    return NextResponse.json({ error: 'Failed to tag basic recipes' }, { status: 500 });
  }
}
