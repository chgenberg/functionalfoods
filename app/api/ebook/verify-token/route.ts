...
export async function POST(request: NextRequest) {
  try {
    ...
    if (new Date() > downloadToken.expiresAt) {
      return NextResponse.json(
        { valid: false, error: 'Nedladdningskoden har gått ut. Kontakta oss på info@functionalfoods.se för hjälp.' },
        { status: 410 }
      );
    }
    ...
    if (downloadToken.downloadCount >= downloadToken.maxDownloads) {
      return NextResponse.json(
        { valid: false, error: 'Max antal nedladdningar uppnått. Kontakta oss på info@functionalfoods.se för hjälp.' },
        { status: 403 }
      );
    }
    ...
    let downloadPath = '';
    if (downloadToken.ebookId === 'julbok-2025') {
      downloadPath = '/Julbok/Functional_foods_julbord_2025.pdf';
    }
    ...
    console.log(`📥 E-book download: ${downloadToken.ebookName} for ${downloadToken.customerEmail} (download ${downloadToken.downloadCount + 1}/${downloadToken.maxDownloads})`);

    return NextResponse.json({
      valid: true,
      ebookName: downloadToken.ebookName,
      downloadPath,
      downloadsRemaining: downloadToken.maxDownloads - downloadToken.downloadCount - 1
    });
