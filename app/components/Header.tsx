...
export default function Header() {
  ...
  const [searchType, setSearchType] = useState<
    "all" | "recipe" | "article" | "raw-material"
  >("all");
  ...
  const menuItems = [
    ...
    { label: t("nav.book", "BÖCKER"), href: "/e-bocker" },
    {
      ...
      submenu: [
        ...
        {
          label: "Functional Insulin balance/Energy",
    ...
    {
      label: t("nav.about", "Om oss"),
      href: "/om-oss",
      submenu: [
        { label: t("nav.about", "Om oss"), href: "/om-oss" },
        {
          label: t("nav.contact", "Kontakta oss"),
        ...
        try {
          ...
          if (purchasesRes.ok) {
            ...
            if (purchases.length > 0) {
              const ownedCourses = purchases.map(
                (p: any) => p.course?.name || "",
              );

              const hasFlow = ownedCourses.some(
                (name: string) =>
                  name.includes("Flow") || name.includes("Gut Health"),
              );
              const hasBasics = ownedCourses.some((name: string) =>
                name.includes("Basics"),
              );
              const hasEnergy = ownedCourses.some(
                (name: string) =>
                  name.includes("Energy") || name.includes("Insulin"),
              );
              const hasHormone = ownedCourses.some(
                (name: string) =>
                  name.includes("Hormonell") || name.includes("Balans"),
              );
  ...
  return (
    <header className="fixed top-0 left-0 right-0 w-full z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="container-custom">
        <div className="relative flex items-center h-24 md:h-28">
          ...
          <div className="lg:hidden">
            ...
            >
              <div className="w-6 h-5 relative flex flex-col justify-between">
                <span
                  className={`block h-0.5 w-full bg-white transform transition-all duration-300 origin-left ${mobileMenuOpen ? "rotate-45 translate-x-px" : ""}`}
                ></span>
                <span
                  className={`block h-0.5 w-full bg-white transition-all duration-300 ${mobileMenuOpen ? "opacity-0 scale-x-0" : ""}`}
                ></span>
                <span
                  className={`block h-0.5 w-full bg-white transform transition-all duration-300 origin-left ${mobileMenuOpen ? "-rotate-45 translate-x-px" : ""}`}
                ></span>
          ...
          <nav className="hidden lg:flex items-center gap-6 ml-6">
            {menuItems.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className="flex items-center space-x-1 text-black hover:text-[#014421] px-2 py-2 text-sm font-semibold tracking-wide transition-colors duration-200"
                >
                ...
                {item.submenu && (
                  <div
                    className={`absolute left-0 mt-2 min-w-[14rem] rounded-xl shadow-xl bg-white ring-1 ring-black/5 transition-all duration-200 ${activeDropdown === item.label ? "opacity-100 translate-y-0 visible" : "opacity-0 -translate-y-2 invisible"}`}
                  >
                      ...
                      {item.submenu.map((subItem) => {
                        ...
                        return (
                          <Link
                            ...
                            className="block px-4 py-2.5 text-sm text-black hover:bg-background-secondary hover:text-[#014421] transition-colors duration-200"
                            role="menuitem"
                          >
                            {subItem.label}
                          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-4 relative z-[60]">
            <button
              className="rounded-full p-2 hover:bg-primary/10 transition"
              aria-label={t("nav.search", "Sök")}
            ...
            {user ? (
              ...
              <button
                ...
                aria-label={t("auth.login", "Logga in")}
              >
            ...
            <Link
              ...
              aria-label={t("cart.label", "Varukorg")}
            >
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-scale-in">
            ...
            <div className="pt-10 pb-4 px-8 text-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-1">
                {tab === "login"
                  ? t("auth.welcomeBack", "Välkommen tillbaka")
                  : t("auth.createAccount", "Skapa konto")}
              </h1>
              <p className="text-sm text-gray-600">
                {tab === "login"
                  ? t("auth.loginToContinue", "Logga in för att fortsätta")
                  : t("auth.startJourney", "Börja din hälsoresa idag")}
              </p>
            </div>
            <div className="px-6 pb-6">
              <div className="flex mb-4 bg-gray-100/50 rounded-full p-0.5">
                <button
                  ...
                  className={`flex-1 py-2 px-4 rounded-full font-medium transition-all duration-300 text-sm ${tab === "login" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-4 rounded-full font-medium transition-all duration-300 text-sm ${tab === "signup" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}
                >
              ...
              {tab === "login" ? (
                <form className="space-y-4" onSubmit={handleLogin}>
                  <div className="relative">
                    ...
                    <input
                      type="email"
                      placeholder={t("auth.email", "E-postadress")}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t("auth.password", "Lösenord")}
      ...
      {showSearch && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-start justify-center pt-20"
          ...
          >
            <div className="p-6 border-b border-gray-100">
              ...
              <div className="flex gap-2">
                {[
                  { id: "all", label: t("search.all", "Allt") },
                  { id: "recipe", label: t("search.recipes", "Recept") },
                  { id: "article", label: t("search.articles", "Artiklar") },
                  {
                    id: "raw-material",
                    label: t("search.ingredients", "Råvaror"),
                  },
                ].map((type) => (
                  <button
                    key={type.id}
                    onClick={() => setSearchType(type.id as any)}
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${searchType === type.id ? "bg-primary text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                  >
                    {type.label}
                  </button>
                ))}
            ...
            <div className="p-6 max-h-96 overflow-y-auto">
              {q.trim() ? (
                results.length > 0 ? (
                    ...
                    {results.map((result) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div
                            className={`w-10 h-10 rounded-lg flex items-center justify-center text-white text-sm font-bold ${result.type === "recipe" ? "bg-green-500" : result.type === "article" ? "bg-blue-500" : "bg-purple-500"}`}
                          >
                            {result.type === "recipe"
                              ? "🍽️"
                              : result.type === "article"
                                ? "📖"
                                : '<Leaf className="w-5 h-5 inline" />'}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <span
                                className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${result.type === "recipe" ? "bg-green-100 text-green-800" : result.type === "article" ? "bg-blue-100 text-blue-800" : "bg-purple-100 text-purple-800"}`}
                              >
                                {result.type === "recipe"
                                  ? t("search.recipe", "Recept")
                                  : result.type === "article"
                                    ? t("search.article", "Artikel")
                                    : t("search.ingredient", "Råvara")}
                              </span>
                              {result.isPremium && (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">
                      {t("search.noResults", "Inga resultat hittades")}
                    </p>
                  </div>
                )
              ) : (
                <div className="text-center py-8">
                  <Search className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                  <p className="text-gray-500">
                    {t("search.startTyping", "Börja skriv för att söka...")}
                  </p>
                </div>
              )}
            </div>
