...
export default function RecipeCarousel() {
  ...
  const moveCarousel = async (direction: 'left' | 'right') => {
    ...
    if (newIndex < 0) {
      newIndex = halfLength - 1;
    } else if (newIndex >= halfLength) {
      newIndex = 0;
    }
    ...
    // Återuppta automatisk animation efter 5 sekunder
    setTimeout(() => {
      setIsPaused(false);
    }, 5000);
  };
    ...
    <section className="py-8 md:py-12 overflow-hidden bg-background-secondary">
      ...
      >
        ...
        <button
          ...
          className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-white text-gray-800 p-2 md:p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 opacity-70 md:opacity-0 group-hover:opacity-100 hover:scale-110"
        ...
        <button
          ...
          className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-white text-gray-800 p-2 md:p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 opacity-70 md:opacity-0 group-hover:opacity-100 hover:scale-110"
          aria-label="Nästa recept"
        >
          <FiChevronRight className="w-5 h-5 md:w-6 md:h-6" />
        </button>

        {/* Gradient overlays för bättre visuell effekt */}
            ...
            >
              ...
              >
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-800 mb-2 line-clamp-2 group-hover/card:text-primary transition-colors">
                      {recipe.title}
                    </h3>
