  ...
  return paragraphs.map((paragraph, index) => {
    ...
    if (/^\d+\./.test(paragraph.trim())) {
      return (
        <div
          key={index}
          className="mb-4 pl-4 border-l-4 border-blue-200 bg-blue-50 p-4 rounded-r-lg"
        >
    ...
    if (paragraph === paragraph.toUpperCase() || paragraph.endsWith(":")) {
      return (
        <h3
          key={index}
          className="text-xl font-bold text-gray-900 mt-8 mb-4 border-b-2 border-border pb-2"
        >
          {paragraph}
        </h3>
      );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        >
          <div
            className={`inline-block px-4 py-2 rounded-full text-sm font-medium text-white bg-gradient-to-r ${article.color} mb-4`}
          >
            {article.category}
          </div>
