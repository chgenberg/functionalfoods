  ...
  return (
    <div className="min-h-screen bg-[#F3EFE3]">
      ...
      <WeekHeroWithVideo
        weekNumber={3}
        weekTitle={weekTitle}
        weekSubtitle="Fortsätt din resa med mer avancerade recept"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-4">
            {getDaysForWeek(3).map((day) => (
              ...
              >
                ...
                `}>
                  {day.current ? 'Idag' : formatDate(3, day.day).split('.')[0] + ' ' + formatDate(3, day.day).split(' ')[1]}
                </div>
                <div className="flex flex-col items-center text-center">
                  ...
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 overflow-hidden relative ${day.completed ? 'bg-green-100' : day.current ? 'bg-white' : 'bg-gray-100'}`}>
                    {dayImages["3"]?.[day.day.toString()] ? (
                      <>
                        <Image
                          src={dayImages["3"][day.day.toString()]!}
