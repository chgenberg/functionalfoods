...
export default function Week4Page() {
  ...
  const weekTitle = "Förfina dina vanor";
  ...
  return (
    <div className="min-h-screen bg-[#F3EFE3]">
      ...
      <WeekHeroWithVideo
        weekNumber={4}
        weekTitle={weekTitle}
        weekSubtitle="Utveckla dina färdigheter ytterligare"
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-4">
            {getDaysForWeek(4).map((day) => (
              <div key={day.day}
                className={`relative p-4 md:p-6 rounded-2xl transition-all cursor-pointer ${day.current ? 'bg-[#FFB5A7] shadow-xl scale-105' : day.completed ? 'bg-white hover:shadow-lg' : day.locked ? 'bg-gray-50 opacity-60' : 'bg-white hover:shadow-lg'}`}
              ...
              >
                ...
                `}>
                  {day.current ? 'Idag' : formatDate(4, day.day).split('.')[0] + ' ' + formatDate(4, day.day).split(' ')[1]}
                </div>
                <div className="flex flex-col items-center text-center">
                  <span className="text-sm text-gray-600 mb-1">{formatDate(4, day.day)}</span>
                  ...
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 overflow-hidden relative ${day.completed ? 'bg-green-100' : day.current ? 'bg-white' : 'bg-gray-100'}`}>
                    {dayImages["4"]?.[day.day.toString()] ? (
                      <>
                        <Image
                          src={dayImages["4"][day.day.toString()]!}
