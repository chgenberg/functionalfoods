      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
            {getDaysForWeek(5).map((day) => (
              <div key={day.day} className={`relative p-6 rounded-2xl transition-all ${day.current ? 'bg-gradient-to-br from-[#FFB5A7] to-[#FCD5CE] shadow-xl scale-105' : day.completed ? 'bg-white hover:shadow-lg' : day.locked ? 'bg-gray-50 opacity-60' : 'bg-white hover:shadow-lg'}`} onClick={() => (window.location.href = `/dashboard/courses/functional-basics/week/5/day/${day.day}`)}>
          ...
          {Object.entries(mealPlan).map(([dayName, dayMeals]: [string, any]) => (
            <div key={dayName} className="border-b border-gray-200 pb-4 mb-4 last:border-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {dayMeals.breakfast && (<div className="bg-orange-50 rounded-lg p-4"><div className="flex items-center gap-2 mb-1"><span className="font-semibold text-[#014421]">Frukost</span><span className="text-xs text-gray-500 bg-orange-100 px-2 py-0.5 rounded-full">07:00</span></div><p className="text-sm text-gray-700">{dayMeals.breakfast.name}</p>{dayMeals.breakfast.slug && (<Link href={`/kunskapsbank/recept/${dayMeals.breakfast.slug}`} className="text-sm text-orange-600 hover:underline mt-1 inline-block">Se recept →</Link>)}</div>)}
                {dayMeals.lunch && (<div className="bg-green-50 rounded-lg p-4"><div className="flex items-center gap-2 mb-1"><span className="font-semibold text-[#014421]">Lunch</span><span className="text-xs text-gray-500 bg-green-100 px-2 py-0.5 rounded-full">12:00</span></div><p className="text-sm text-gray-700">{dayMeals.lunch.name}</p>{dayMeals.lunch.slug && !dayMeals.lunch.name.includes('rester') && (<Link href={`/kunskapsbank/recept/${dayMeals.lunch.slug}`} className="text-sm text-green-600 hover:underline mt-1 inline-block">Se recept →</Link>)}</div>)}
                {dayMeals.dinner && (<div className="bg-purple-50 rounded-lg p-4"><div className="flex items-center gap-2 mb-1"><span className="font-semibold text-[#014421]">Middag</span><span className="text-xs text-gray-500 bg-purple-100 px-2 py-0.5 rounded-full">18:00</span></div><p className="text-sm text-gray-700">{dayMeals.dinner.name}</p>{dayMeals.dinner.slug && (<Link href={`/kunskapsbank/recept/${dayMeals.dinner.slug}`} className="text-sm text-purple-600 hover:underline mt-1 inline-block">Se recept →</Link>)}</div>)}
              </div>
            </div>
          ))}
        </div>
