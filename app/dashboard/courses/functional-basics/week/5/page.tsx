        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-4">
            {getDaysForWeek(5).map((day) => (
              <div key={day.day} className={`relative p-4 md:p-6 rounded-2xl transition-all ${day.current ? 'bg-[#FFB5A7] shadow-xl scale-105' : day.completed ? 'bg-white hover:shadow-lg' : day.locked ? 'bg-gray-50 opacity-60' : 'bg-white hover:shadow-lg'}`} onClick={() => (window.location.href = `/dashboard/courses/functional-basics/week/5/day/${day.day}`)}>
