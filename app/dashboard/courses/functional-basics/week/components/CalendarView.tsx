...
export function CalendarView({ mealPlan, weekNumber }: CalendarViewProps) {
  ...
  const [miniChecklist, setMiniChecklist] = useState<Record<string, boolean>>(()=>{
    try { return JSON.parse(localStorage.getItem(`miniChecklist_week_${weekNumber}`) || '{}'); } catch { return {}; }
  });
  ...
  useEffect(()=>{
    try { localStorage.setItem(`miniChecklist_week_${weekNumber}`, JSON.stringify(miniChecklist)); } catch {}
  }, [miniChecklist, weekNumber]);

  const defaultTasks = [
    { id: 'prep', label: 'Måltidsprepp söndagskväll' },
    { id: 'move', label: '30 min rörelse idag' },
    { id: 'sleep', label: 'Sömnrutin i kväll' },
    ...
    <div className="space-y-6">
      ...
      <div className="sticky top-16 z-20">
        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow p-3 flex items-center justify-between">
          ...
          <button
            onClick={() => setSelectedDay(currentDayIndex >= 0 ? currentDayIndex : 0)}
            className="px-3 py-1.5 rounded-full bg-primary text-white text-sm hover:bg-secondary"
          >
            Idag
          </button>
        </div>
