...
async function main() {
  ...
  try {
    ...
    // Set course start date to today for immediate access
    const courseStartDate = new Date();
    
    // Check if course start record exists
    const existingCourseStart = await prisma.userCourse.findFirst({
      where: {
        userId: user.id,
        courseType: 'functional-energy'
      }
    });
    
    if (existingCourseStart) {
      await prisma.userCourse.update({
        where: { id: existingCourseStart.id },
        data: { startDate: courseStartDate }
      });
    } else {
    ...
    console.log('📅 Course start date set to today');
    ...
    console.log(`📅 Start date: ${courseStartDate.toLocaleDateString('sv-SE')}`);
    console.log('\n🔗 Login at: /login');
