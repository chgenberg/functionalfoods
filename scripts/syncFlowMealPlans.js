...
// Main function to sync Flow meal plans
async function syncFlowMealPlans() {
  const prisma = new PrismaClient();
  console.log('🔄 Starting Flow meal plan sync...');
  ...
  // 2. Define Flow DOCX files to process
  const docxFiles = [
    'Functionalflow_1.docx', 'Functionalflow_2.docx', 'Functionalflow_3.docx',
    'Functionalflow_4.docx', 'Functionalflow _5.docx', 'Functionalflow_6.docx',
  ];
  const docxDir = path.resolve(__dirname, '../public/kurser/flow');
  
  const generatedFlowMealPlans = {};
  const missingRecipes = new Set();

  // 3. Process each Flow DOCX file
  for (let i = 0; i < docxFiles.length; i++) {
    ...
    console.log(`\n--- Processing Flow Week ${weekNum} (${fileName}) ---`);
    ...
    const weekPlan = {
      title: `Vecka ${weekNum}: Flow synkroniserad från DOCX`,
      days: {},
    };
    ...
    generatedFlowMealPlans[`week${weekNum}`] = weekPlan;
  }

  // 4. Save the output
  const outputPath = path.resolve(__dirname, 'generatedFlowMealPlans.json');
  fs.writeFileSync(outputPath, JSON.stringify(generatedFlowMealPlans, null, 2));
  console.log(`\n\n✅ Flow meal plan data successfully generated and saved to ${outputPath}`);
  
  // 5. Report missing recipes
  if (missingRecipes.size > 0) {
    console.log('\n\n⚠️ The following Flow recipes from DOCX were not found in the database:');
    missingRecipes.forEach(r => console.log(`  - ${r}`));
    console.log('\nThese have been added to the meal plan as text-only (no link).');
    
    // Save missing recipes list for next step
    const missingArray = Array.from(missingRecipes).filter(r => r.length > 2);
    fs.writeFileSync(path.resolve(__dirname, 'missingFlowRecipes.json'), JSON.stringify(missingArray, null, 2));
  } else {
    console.log('\n\n✅ All Flow recipes from DOCX were successfully matched in the database!');
  }

  await prisma.$disconnect();
}

syncFlowMealPlans().catch(console.error); 