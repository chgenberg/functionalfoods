...
const BASE_DIR = path.resolve(process.cwd(), 'Recept-Final', 'Kostscheman-flow');
...
(async () => {
  try {
    ...
    // Read current mealPlans.ts
    const currentContent = fs.readFileSync(MEAL_PLANS_PATH, 'utf8');
    
    // Find flowMealPlans section
    const flowStartMatch = currentContent.match(/(^\/\/ Functional Flow meal plans[\s\S]*?export const flowMealPlans[\s\S]*?= \{)/m);
    if (!flowStartMatch) {
      throw new Error('Could not find flowMealPlans structure in file');
    }
    
    const beforeFlow = currentContent.substring(0, flowStartMatch.index);
    const flowPrefix = flowStartMatch[1];
    
    // Find end of flowMealPlans
    const flowEndMatch = currentContent.match(/^(\/\/ Helper function[\s\S]*)/m);
    const suffix = flowEndMatch ? flowEndMatch[1] : '';

    // Generate new content
    const newContent = `${beforeFlow}${flowPrefix}
...
${suffix}`;
    ...
    console.log('✅ Updated flowMealPlans in app/data/mealPlans.ts with Flow schedules');
    ...
    console.error('❌ Flow sync failed:', e.message || e);
    process.exit(1);
  }
})(); 