...
function parseMealPlans() {
  ...
  // Extract the three meal plan objects using regex
  const mealPlansMatch = content.match(/export const mealPlans: Record<string, WeekMealPlan> = ({[\s\S]*?});/);
  const flowMealPlansMatch = content.match(/export const flowMealPlans: Record<string, WeekMealPlan> = ({[\s\S]*?});/);
  const energyMealPlansMatch = content.match(/export const energyMealPlans: Record<string, WeekMealPlan> = ({[\s\S]*?});/);
  
  let mealPlans = null;
  let flowMealPlans = null;
  let energyMealPlans = null;
  
  try {
    if (mealPlansMatch) {
      mealPlans = eval('(' + mealPlansMatch[1] + ')');
    }
    if (flowMealPlansMatch) {
      flowMealPlans = eval('(' + flowMealPlansMatch[1] + ')');
    }
    if (energyMealPlansMatch) {
      energyMealPlans = eval('(' + energyMealPlansMatch[1] + ')');
    }
  } catch (error) {
...
async function verifyAllCourseRecipes() {
  ...
  try {
    ...
    // Parse meal plans
    const { mealPlans, flowMealPlans, energyMealPlans } = parseMealPlans();
